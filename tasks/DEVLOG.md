...
Designed a Claude Code session workflow centered on "GitHub is source of truth, session is ephemeral worker":

1. **ONE named session per project** (`/rename [project-name]`)
2. **Canonical host = tmux on athena** (homelab never sleeps, survives disconnects)
3. **Cross-device via `/remote-control`** (relay running session to another device) or `/teleport` (pull web session into CLI)
4. **Per-project `CLAUDE.md`** generated by `/init`, hand-edited, kept <200 lines
5. **PreCompact hook** auto-writes DEVLOG marker before context collapse
6. **L1/L2/L3 layer model** validated:
   - **L1 Hermes** — cron, homelab, Telegram, project-init (always-on orchestrator)
   - **L2 Claude Code** — interactive coding (CLI/Desktop/Web/VS Code extension)
   - **L3 Opencode/Codex/Copilot/Gemini** — inline helpers

**Files created/updated:**
- `~/dev-shared/workflow/SESSION-WORKFLOW.md` (392 lines) — full session lifecycle, compaction handling, sync model
- `~/dev-shared/workflow/AI-ROUTING.md` — added Layer model section + CLAUDE.md in context file hierarchy
- `~/dev-shared/workflow/WORKFLOW-ORCHESTRATION.md` — mental tree references SESSION-WORKFLOW.md
- `tasks/lessons.md` — **L-064** added: don't invent CLI command names, verify with actual tool

**State:**
- All three workflow files updated
- Memory entries corrected (replaced `/rc` hallucination with `/remote-control`)
- L1/L2/L3 layer model documented in AI-ROUTING.md
- Cross-device session mental model now matches Claude Code's actual capabilities (verified via direct user conversation with Claude Code)

**Decided:**

- **Hermes stays L1 only** — cron/orchestration, not interactive coding. Confirmed by Claude Code review that Hermes's context reconstruction from ~/.hermes/ markdown causes drift, path hallucination, and unsupervised sub-agent dispatch. Use Claude Code for any coding work.
- **Athena tmux = canonical session host** — Desktop app/Web/VS Code are guest surfaces. CLI sessions are local-only per machine.
- **CLAUDE.md is load-bearing** — Claude Code reads it at session start (different from `CONTEXT.md` which is agent-agnostic, and `AGENTS.md` which is the behavior contract).
- **Sub-agents via `Task` tool** for contained work only — don't persist across parent resumes.

**Blocked / Next:**

- **Generate `CLAUDE.md` for active projects** — `/init` should be run once per active project repo to bootstrap the file
- **Add PreCompact hook** — paste into `~/.claude/settings.json` so DEVLOG captures pre-compaction state
- **Hands-on validation** — try the workflow with one project end-to-end (start session in tmux, /init, /plan, work, commit, push, exit, resume from Mac) before declaring it the standard
- **Update memory** — done (replaced wrong `/rc` entries with corrected `/remote-control` + L1/L2/L3 model)
